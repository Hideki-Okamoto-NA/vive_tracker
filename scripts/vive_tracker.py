...
def vive_tracker():
    ...
    try:
        ...
        if (
            ...
            and ex.args[0]
            == "VRInitError_Init_HmdNotFoundPresenceFailed (error number 126)"
        ):
            ...
            print(
                "Are the Lighthouse Base Stations powered and in view of the Tracker?\n\n"
            )
        else:
    ...
    p_cov[0, :] = [
        0.0000349162103240595,
        -0.0000018202960310455,
        -0.0000339898160507969,
        -0.0000081126791170800,
        0.0000001353045808767,
        0.0000032202291901186,
    ]
    p_cov[1, :] = [
        -0.0000018202960310455,
        0.0000011910722363973,
        0.0000020423436706964,
        0.0000010961526869235,
        -0.0000000333091396801,
        -0.0000001408541892558,
    ]
    p_cov[2, :] = [
        -0.0000339898160507969,
        0.0000020423436706964,
        0.0000341312090595451,
        0.0000060715616751347,
        -0.0000000237628610568,
        -0.0000029217229365340,
    ]
    p_cov[3, :] = [
        -0.0000081126791170800,
        0.0000010961526869235,
        0.0000060715616751347,
        0.0000165832615351042,
        -0.0000004759697840205,
        -0.0000024486872043021,
    ]
    p_cov[4, :] = [
        0.0000001353045808767,
        -0.0000000333091396801,
        -0.0000000237628610568,
        -0.0000004759697840205,
        0.0000003366392930324,
        -0.0000000030521109214,
    ]
    p_cov[5, :] = [
        0.0000032202291901186,
        -0.0000001408541892558,
    ...
    while not rospy.is_shutdown():
        ...
        for deviceName in v.devices:
            ...
            if "LHR" in v.devices[deviceName].get_serial():
                [qx, qy, qz, qw] = tf.transformations.quaternion_multiply(
                    [qx, qy, qz, qw],
            ...
            broadcaster[deviceName].sendTransform(
                (x, y, z), (qx, qy, qz, qw), time, publish_name_str, "vive_world"
            )
            ...
            if deviceName not in publisher:
                publisher[deviceName] = rospy.Publisher(
                    publish_name_str, String, queue_size=10
                )

            publisher[deviceName].publish(
                "  X: "
                + str(x)
                + "  Y: "
                + str(y)
            ...
            if "reference" not in deviceName:
                if deviceName + "_odom" not in publisher:
                    publisher["ring_odom"] = rospy.Publisher(
                        "ring_odom", Odometry, queue_size=50
                    )
                    publisher[deviceName + "_odom"] = rospy.Publisher(
                        publish_name_str + "_odom", Odometry, queue_size=50
                    )
                    publisher[deviceName + "_inverted_odom"] = rospy.Publisher(
                        publish_name_str + "_inverted_odom", Odometry, queue_size=50
                    )
                    publisher[deviceName + "_pose"] = rospy.Publisher(
                ...
                [vx, vy, vz, v_roll, v_pitch, v_yaw] = v.devices[
                    deviceName
                ].get_velocities()
                odom.twist.twist = Twist(
                    Vector3(vx, vy, vz), Vector3(v_roll, v_pitch, v_yaw)
                )
                ...
                if not (
                    ...
                    try:
                        (ring_trans, ring_rot) = listener.lookupTransform(
                            "vive_world", "ring", time
                        )
                        odom.pose.pose = Pose(
                            Point(ring_trans[0], ring_trans[1], ring_trans[2]),
